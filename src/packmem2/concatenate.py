...
def start_file(prefix: str, suffix: str, start: int, end: int) -> tuple[str, int]:
    """
...
def concat_files(prefix: str, suffix: str, start: int, end: int) -> pd.DataFrame:
    """
    ...
    for pdbnum in range(start + 1, end + 1):
        if check_file(f"{prefix}{pdbnum}{suffix}"):
            file = pd.read_csv(
                f"{prefix}{pdbnum}{suffix}", sep=r"\s+", header=None, skiprows=[0, 1]
            )
...
def concat_files_prot(prefix: str, suffix: str, start: int, end: int) -> pd.DataFrame:
    """
...
def launch(output_dir: str, prefix: str, start: int, end: int, prot: bool) -> None:
    ...
    if prot:
        if Path(f"{output_dir}/Prot_{prefix}0_Up_All.txt").is_file():
            ...
            Total_Up_Deep_prot.to_csv(
                f"{output_dir}/Total_Up_Deep_prot.csv", header=False, index=False
            )
            Total_Up_All_prot.to_csv(
                f"{output_dir}/Total_Up_All_prot.csv", header=False, index=False
            )
            ...
            Total_Lo_Deep_prot.to_csv(
                f"{output_dir}/Total_Lo_Deep_prot.csv", header=False, index=False
            )
            Total_Lo_All_prot.to_csv(
                f"{output_dir}/Total_Lo_All_prot.csv", header=False, index=False
            )
    ...
    Total_Shallow = pd.concat(
        [Total_Up_Shallow, Total_Lo_Shallow], axis=0, ignore_index=True
    )
    ...
    Total_Up_Shallow.to_csv(
        f"{output_dir}/Total_Up_Shallow.csv", header=False, index=False
    )
    Total_Lo_Shallow.to_csv(
        f"{output_dir}/Total_Lo_Shallow.csv", header=False, index=False
    )
